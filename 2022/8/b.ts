        ...
        treeRow
            .map((tree, treeIndex) => {
                ...
                for (let i = 1; i < treeRow.length - treeIndex; i++) {
                    const b = treeRow[treeIndex + i];
                    if (b >= tree || i === treeRow.length - treeIndex - 1) {
                ...
                for (let i = 1; i < rowsArr.length - rowIndex; i++) {
                    const b = rowsArr[rowIndex + i][treeIndex];
                    if (b >= tree || i === rowsArr.length - rowIndex - 1) {
                        visibleTreesToDown = i;
                        break;
                    }
                }

                return (visibleTreesToLeft * visibleTreesToRight * visibleTreesToUp * visibleTreesToDown);
