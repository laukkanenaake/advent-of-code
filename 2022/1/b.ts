...
inputArray.forEach((calory) => {
	...
	const isElfInTopThree = topThreeElvesCaloriesAmounts.some(
		(calories) => calories < caloriesOnCurrentElf,
	);
	if (!isElfInTopThree) {
...
const sumOfTopThree = topThreeElvesCaloriesAmounts.reduce(
	(prev, curr) => prev + curr,
);

console.log(
	"sum of the calories of the top three elves with the most calories: ",
	sumOfTopThree,
);
