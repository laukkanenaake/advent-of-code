...
const getDirectorySize: (directory: Directory) => number = (directory: Directory) => {
    ...
    const sizeIncludingDirectories = [...directory.files, ...directorySizes].reduce((acc, curr) => acc + curr);
    const sizeWithoutDirectories = directory.files.length ? directory.files.reduce((acc, curr) => acc + curr) : 0;
...
const usedSpaceOnDisk = directorySizesWithoutDirectories.reduce((acc, curr) => acc + curr);
