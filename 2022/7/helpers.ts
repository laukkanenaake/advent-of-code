...
export const getFiles: () => Promise<Directory> = async () => {
    ...
    inputText.split("\n$ ").forEach(line => {
        if (line[0] === "c") {
            ...
            currentDirectory = currentDirectory.directories.get(dirName) ?? currentDirectory;
            ...
            contents.forEach(dirContent => {
                ...
                if (dirContent[0] === "d") {
                    ...
                    if (!currentDirectory.directories.has(dirName)) {
                        currentDirectory.directories.set(dirName, new Directory(currentDirectory))
                    }
                } else {
                    directoryFiles.push(parseInt(dirContent.substring(0, spaceIndex)));
                }
            })
