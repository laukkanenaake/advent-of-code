...
const numberStrings = ["one","two","three","four","five","six","seven","eight","nine","1","2","3","4","5","6","7","8","9"];
...
const result = inputText.split("\n")
    .map(string => {
        ...
        numberStrings.forEach(number => {
            ...
            if (firstNumber.index === undefined || numberFirstIndex < firstNumber.index) {
                firstNumber = {
                    index: numberFirstIndex,
                    value: isNaN(parseInt(number)) ? (numberStrings.indexOf(number) + 1).toString() : number
                };
            }
            if (lastNumber.index === undefined || numberLastIndex > lastNumber.index) { 
                lastNumber = {
