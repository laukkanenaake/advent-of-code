...
inputText
    ...
    .forEach((row, rowIndex, rows) => {
        ...
        const checkAdjacentGear = (charIndex: number, rowIndex: number) => {
            if (rowIndex < 0 || charIndex < 0 || rowIndex >= rows.length || charIndex >= row.length) {
                return false;
            }
                    ...
                    return offsets.forEach(rowOffset => (
                        offsets.forEach(charOffset => {
                            ...
                            if (checkAdjacentGear(x, y)) {
                                ...
                                let adjacentNumbers = [...gearLocations.get(gearLocation) ?? []];
                                const newAdjacentNumber = {
                                    charIndex: num.startIndex, 
                                    rowIndex: rowIndex, 
                                    value: parseInt(num.value)
                                };
                                const isDuplicate = adjacentNumbers.some(number => (
                                    number.charIndex === newAdjacentNumber.charIndex &&
                                    number.rowIndex === newAdjacentNumber.rowIndex &&
...
const gearRatios = Array
    .from(gearLocations.values())
